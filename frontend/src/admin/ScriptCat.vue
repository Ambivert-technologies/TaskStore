    ...
    <div class="col-12">
      <div class="card mb-4">
        <div class="card-header pb-0">
          ...
          <p class="text-sm">New Task Category</p>
        </div>
                        ...
                        <tr v-for="item in catItems" :key="item.id">
                          ...
                          <td class="align-middle text-center cursor-pointer">
                            <p
                              ...
                              @click="editCat(item['_id'], item['cat'])"
                            >
                              ✏️
                            </p>
                          </td>
  ...
  mounted() {
    ...
    if (loggedIn != "true") {
      ...
      if (role != "admin") {
        ...
        appInstance.axios
          .get(
            "https://yh3aetiwp1.execute-api.ap-south-1.amazonaws.com/dev/app/v1/users/getCat",
            // "http://127.0.0.1:8000/app/v1/users/getCat",
            {
  ...
  methods: {
    formSubmit(event) {
      ...
      if (cat.trim() != "") {
        appInstance.axios
          .post(
            "https://yh3aetiwp1.execute-api.ap-south-1.amazonaws.com/dev/app/v1/users/newScriptCat",
            // "http://127.0.0.1:8000/app/v1/users/newScriptCat",
            formData,
            {
              headers: { "Content-Type": "application/json" },
            }
    ...
    editCatFormSubmit(event){
      event.preventDefault();
      var formData = new FormData();
      formData.append("ecat", this.ecat);
      formData.append("ecatId", this.ecatId);
      appInstance.axios
        .post("https://yh3aetiwp1.execute-api.ap-south-1.amazonaws.com/dev/app/v1/users/editCat", formData, {
        // .post("http://127.0.0.1:8000/app/v1/users/editCat", formData, {
          headers: { "Content-Type": "application/json" },
        })
        .then((res) => {
          if (res.data["msg"] === true) {
    ...
    delCat(catId) {
      ...
      appInstance.axios
        .post("https://yh3aetiwp1.execute-api.ap-south-1.amazonaws.com/dev/app/v1/users/delCat", formData, {
        // .post("http://127.0.0.1:8000/app/v1/users/delCat", formData, {
          headers: { "Content-Type": "application/json" },
        })
        .then((res) => {
          if (res.data["msg"] === true) {
